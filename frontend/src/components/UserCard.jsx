    ...
    return (
        <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
            <div className='text-center'>
                ...
                {user.bio && <p className='text-gray-600 mt-2 text-center text-sm px-2'>{user.bio}</p>}
            </div>
            ...
            <div className='flex mt-4 gap-2'>
                ...
                <button 
               onClick={handleConnectionRequest} className='flex items-center justify-center w-16 border text-slate-600 group rounded-md cursor-pointer active:scale-95 transition'>
